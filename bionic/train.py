...
class Trainer:
    ...
    def forward(self, verbosity: Optional[int] = 1):
        ...
        train_emb_df.to_csv(extend_path(self.params.out_name, "_train__features.tsv"), sep="\t", index_label='patients')
        test_emb_df.to_csv(extend_path(self.params.out_name, "_test__features.tsv"), sep="\t", index_label='patients')
        last_layer_emb = np.concatenate(last_layer_emb_list)
        last_layer_emb_df = pd.DataFrame(last_layer_emb, index=self.index)
        last_layer_emb_df.to_csv(extend_path(self.params.out_name, "_last_layer_features.tsv"), sep="\t", index_label='patients')
